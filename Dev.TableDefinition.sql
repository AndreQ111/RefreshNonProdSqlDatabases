-- Dev.TableDefinition.sql
	-- works, includes identity and PK and all indices
	-- https://www.stormrage.com/SQLStuff/sp_GetDDL_Latest.txt
	-- https://www.sqlservercentral.com/forums/topic/get-ddl-for-any-sql-2005-table
...
--#################################################################################################
-- USAGE: exec TableDefinition GMACT
--   or   exec TableDefinition 'bob.example'
--   or   exec TableDefinition 'temp.DeleteMe'
--   or   exec TableDefinition #temp
--#################################################################################################
...
--v319	 Andre Quitta 3/1/22 removed everything on Fill_Factor conditions, to accept default of 0
DROP PROCEDURE IF EXISTS [Dev].[TableDefinition]
go
--#############################################################################
...
--EXECUTE sp_ms_marksystemobject 'TableDefinition'
--#############################################################################
CREATE PROCEDURE [Dev].[TableDefinition]
...
exec Dev.TableDefinition @SchemaTableName = 'Schema.TableName', @CreateIndex = 0


--select @Sql = Item
select	*
from	#TableDef
--set @Sql = replace(@Sql,'Staging', 'Temp' )		-- be careful with this
print @Sql
...
--##############################################################################
  ...
  --  SELECT @ISSYSTEMOBJECT = ISNULL(is_ms_shipped,0),@PROCNAME = ISNULL(name,'##TableDefinition') FROM sys.objects WHERE OBJECT_ID = @@PROCID
  --IF @ISSYSTEMOBJECT IS NULL 
  --  SELECT @ISSYSTEMOBJECT = ISNULL(is_ms_shipped,0),@PROCNAME = ISNULL(name,'##TableDefinition') FROM master.sys.objects WHERE OBJECT_ID = @@PROCID
  --IF @ISSYSTEMOBJECT IS NULL 
  --  SET @ISSYSTEMOBJECT = 0  
  --IF @PROCNAME IS NULL
    SET @PROCNAME = '##TableDefinition';
  --SET @TBL =  '[DBO].[WHATEVER1]'
  --does the tablename contain a schema?
  SET @vbCrLf =  CHAR(10);
  SELECT @SCHEMANAME = ISNULL(PARSENAME(@SchemaTableName,2),'dbo') ,
         @TBLNAME    = PARSENAME(@SchemaTableName,1);
	SELECT
			@TBLNAME    = [OBJS].[name],
...
--##############################################################################
 SELECT 
   @FINALSQL =  '' --'IF OBJECT_ID(''' + QUOTENAME(@SCHEMANAME) + '.' + QUOTENAME(@TBLNAME) + ''') IS NOT NULL ' + @vbcrlf
              --+ 'DROP TABLE if exists ' + QUOTENAME(@SCHEMANAME) + '.' + QUOTENAME(@TBLNAME) + '; ' + @vbcrlf + 'GO;' + @vbcrlf
...
--##############################################################################
  DECLARE @Results  TABLE (
                    ...
                    [CurrentCompression]    varchar(128) default 'NONE') ;		-- Andre Quitta 2/23/22 because it's never used, messes up fill_factor
IF @CreateIndex <> 0
BEGIN
  INSERT INTO @Results
    SELECT
...
END	-- IF @CreateIndex <> 0
--@Results table has both PK,s Uniques and indexes in thme...pull them out for adding to fInal results:
  SET @CONSTRAINTSQLS = '';
  SET @INDEXSQLS      = '';
...
--Mark as a system object
----EXECUTE sp_ms_marksystemobject 'TableDefinition';
----GRANT EXECUTE ON Dev.TableDefinition TO PUBLIC;
--#################################################################################################
GO